...
public class AcsShortPartitionReliabilityIT extends BaseReliabilityIT
{
    ...
    {
        Node victim;
        log.info("[reliability] Disabling toxic-acs proxy for ~{} s", PARTITION_SECONDS);
        ...
        try
        {
            ...
            victim = environment().repositoryClient()
                    .createNodeWithContent(PARENT_ID, "acs-partition-victim.txt", content, "text/plain");
            log.info("[reliability] Mid-partition node {} published — content download path expected to bounded-fail and DLQ", victim.id());
        ...
        Node sentinel = environment().repositoryClient()
                .createNodeWithContent(PARENT_ID, "acs-partition-sentinel.txt", postContent, "text/plain");
        ...
        RetryUtils.retryWithBackoff(() -> {
            ...
            assertThat(environment().jolokia().topicSubscriberCount(REPO_EVENT_TOPIC))
                    .as("ACS partition must not knock out the JMS subscription — live-ingester should still be subscribed to %s", REPO_EVENT_TOPIC)
                    .isGreaterThanOrEqualTo(1);
