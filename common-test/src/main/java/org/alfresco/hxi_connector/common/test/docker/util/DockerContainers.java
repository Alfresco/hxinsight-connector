...
public class DockerContainers
{
    ...
    {
        // Wait for both the broker's "started" log line and the OpenWire port (61616) to be
        // reachable. The log line alone is not enough: Testcontainers matches the log regex with
        // DOTALL per (potentially multi-line) frame, so during fast startup the banner line
        // ("Apache ActiveMQ x.y.z") plus an early "...started" line (e.g. "PListStore ... started")
        // match before the OpenWire connector binds. Explicitly waiting on port 61616 guarantees
        // the broker is actually accepting connections before the test runs.
        return new GenericContainer<>(DockerImageName.parse(ACTIVE_MQ_IMAGE).withTag(ACTIVE_MQ_TAG))
                ...
                .waitingFor(new WaitAllStrategy()
                        .withStrategy(Wait.forLogMessage(".*Apache ActiveMQ.*started.*\\n", 1))
                        .withStrategy(Wait.forListeningPorts(61616))
                        .withStartupTimeout(Duration.ofMinutes(2)));
    }
    ...
    public static WireMockContainer createWireMockContainer()
    {
        return new WireMockContainer(DockerImageName.parse(WIREMOCK_IMAGE).withTag(WIREMOCK_TAG))
                .withEnv("WIREMOCK_OPTIONS", "--disable-gzip --verbose");
    }
    ...
    public static WireMockContainer createWireMockContainerWithin(Network network)
    {
        return createWireMockContainer()
                .withNetwork(network)
                .withEnv("WIREMOCK_OPTIONS", "--global-response-templating --disable-gzip --verbose");
    }
