...
/**
 * Raised when {@link EventProcessor} receives a syntactically-valid {@code RepoEvent} whose {@code type} matches none of the dispatch predicates ({@code Created} / {@code Updated} / {@code PermissionUpdated} / {@code Deleted} / prediction variants). The default-deployment path swallows the event silently after an INFO log and a {@link LiveIngesterMetrics.Drop#REPO_EVENTS_UNHANDLED} counter increment; deployments that opt into {@code alfresco.repository.events-subscription.dead-letter-unsupported-types=true} re-throw the abandonment as this exception so the route's error handler routes it through the existing repo-events {@code DeadLetterChannel} for a structured operator signal — DLQ entry, exception-tagged DLQ counter, masked exchange-state log line.
 */
