...
public class CamelTest
{
    ...
    public CamelTest(CamelContext camelContext, ProducerTemplate producerTemplate, IntegrationProperties integrationProperties)
    {
        INGESTER_LISTENER = new TestEventListener(
                camelContext,
                integrationProperties.hylandExperience().ingester().endpoint(),
                exchange -> exchange.getMessage().setHeaders(Map.of(HTTP_RESPONSE_CODE, 202)));

        ATS_REQUEST_LISTENER = new TestEventListener(
                camelContext,
